// Inicializar bankValue de forma segura
let bankValue = (typeof saldo !== 'undefined') ? saldo : 0;

// Función global para actualizar el balance desde postMessage
window.updateGameBalance = function (newBalance) {
	...
	console.log('App: Balance actualizado desde parent:', newBalance);
};

// Función helper para enviar saldo
function notificarSaldo() {
	if (typeof enviarSaldo === 'function') {
		enviarSaldo(bankValue);
	}
}
let currentBet = 0;
let wager = 5;
...
function setBet(e, n, t, o) {
	...
	if (wager > 0) {
		...
		document.getElementById('bankSpan').innerText = '' + bankValue.toLocaleString("en-GB") + '';
		document.getElementById('betSpan').innerText = '' + currentBet.toLocaleString("en-GB") + '';
		notificarSaldo(); // Notificar cambio
...
function spin() {
	...
	setTimeout(function () {
		...
		if (numbersBet.includes(winningSpin)) {
			...
			// Calcular total apostado aunque se pierda
			for (i = 0; i < bet.length; i++) {
				betTotal += bet[i].amt;
			}
		}

		// Notificar al padre el resultado del juego para persistencia
		if (window.parent) {
		...
		document.getElementById('bankSpan').innerText = '' + bankValue.toLocaleString("en-GB") + '';
		document.getElementById('betSpan').innerText = '' + currentBet.toLocaleString("en-GB") + '';
		notificarSaldo(); // Notificar cambio tras spin
...
function win(winningSpin, winValue, betTotal) {
	...
	// Enviar resultado al padre para persistencia (independientemente de si ganó o no)
	// Nota: 'win' se llama solo si gana, pero necesitamos minar también si pierde.
	// La lógica original llamaba a win() solo si ganaba.
	// MODIFICACION: Moveremos esta lógica a spin() para cubrir ambos casos o adaptar aquí.
	// Pero como win() solo se llama si gana, aquí solo notificamos victoria.
	// Espera, el código original en spin() llama a win(...) solo si numbersBet.includes(winningSpin).
	// Si pierde, no llama a win().
	// Por lo tanto, debemos modificar spin() en lugar de win() para garantizar reporting.
}
